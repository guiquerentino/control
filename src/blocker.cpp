...
void ApplyRules(const Config& cfg,bool active){
    std::string p=Narrow(HostsPath());
    std::wofstream f(p.c_str(),std::ios::app);
    if(!f.is_open())return;
    for(auto& r:cfg.rules)if(r.site)if(active)f<<L"127.0.0.1 "<<std::wstring(r.target.begin(),r.target.end())<<L"\n";
}
void RemoveSites(const Config& cfg){
    ...
    in.close();
    for(auto&r:cfg.rules)if(r.site){std::wstring s=L"127.0.0.1 "+std::wstring(r.target.begin(),r.target.end())+L"\n";size_t pos;while((pos=data.find(s))!=std::wstring::npos)data.erase(pos,s.size());}
